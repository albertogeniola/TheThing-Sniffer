##! Add a field named port_service to the conn log which contains the name of port 
##! used by the server in the connection.

# Make sure we log as json
@load policy/tuning/json-logs.bro
@load policy/frameworks/files/extract-all-files.bro
const etc_services: table[port] of string &default="unknown" &redef;

redef record Conn::Info += {
        service_guessed: bool &log &optional;
};

event connection_state_remove(c: connection)
{
	if ( ! c$conn?$service || c$conn$service == "") {
	    c$conn$service = etc_services[c$id$resp_p];
	    c$conn$service_guessed = T;
	} else {
	    c$conn$service_guessed = F;		
	}
}
...
# Actually define the etc_services variable here just to make the tiny
# bit of code in this script visible at the top of the script.
redef etc_services = {
	[1/udp] = "tcpmux",
	[1/tcp] = "tcpmux",
	[2/udp] = "compressnet",
